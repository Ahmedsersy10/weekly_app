...
class SettingsService {
  ...
  static Future<SettingsModel> loadSettings() async {
    ...
    final themeMode =
        ThemeMode.values[prefs.getInt(_themeModeKey) ?? 2]; // default to system
    final primaryColorValue = prefs.getInt(_primaryColorKey) ?? 0xFF8E1616;
    ...
    final weekStart =
        WeekStart.values[prefs.getInt(_weekStartKey) ?? 0]; // default to monday
    ...
    final syncProvider = SyncProvider
        .values[prefs.getInt(_syncProviderKey) ?? 2]; // default to none
    final autoSync = prefs.getBool(_autoSyncKey) ?? false;
    final language =
        Language.values[prefs.getInt(_languageKey) ?? 0]; // default to english
  ...
  static Future<void> saveSettings(SettingsModel settings) async {
    ...
    await prefs.setInt(
      _lastBackupKey,
      settings.lastBackup.millisecondsSinceEpoch,
    );
  }
